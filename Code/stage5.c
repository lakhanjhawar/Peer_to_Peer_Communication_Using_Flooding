...
thread_t thr[N+1];
int r_num,nw[N+1][N+1],flag[N+1],num_of_entries_in_queue,status[N+1][N+1],status1[N+1][N+1][N+1];
char line[100];
char token[] = {" ."};
char *ptoken;
FILE *print;
...
int main(int argc, char *argv[])
{
	...
	node_ptr y;
	char buff[20];
	int i,j,count,row,col,c_num,n,status,k,m;
	fp=fopen(argv[1],"r");
	print=fopen("stage5_log.txt","w");
...
if(ttl1>1)
{
	...
	flag[sender1]=1;
	num_of_entries_in_queue=1;
	fprintf(print,"   %d %d %d %d\n ",temp->sender,receiver1,temp->msg,temp->TTL);
	        enterQ(temp,receiver1);
	        return 0;
}
else
