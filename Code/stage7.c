...
thread_t thr[N+1];
int r_num,nw[N+1][N+1],flag[N+1],status[N+1][N+1],status1[N+1][N+1][N+1];
char line[100];
char token[] = {" ."};
char *ptoken;
FILE *print;
...
void *peer(void *arg)
{
	int myid=(int)arg,i,j,msg1,ttl1,sender1,count,came_from,num_rounds_no_msg;
	node_ptr y;
	int myneighbours[N+1],num_empty=N;
	for(i=0;i<=r_num+1;i++)
